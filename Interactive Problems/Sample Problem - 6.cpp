...
// Memory Limit: 256 MB
// Time Limit: 1000 ms
// Parsed on: 20-02-2021 14:18:19 IST (UTC+05:30)
...
void solve()
{
  	...
  	int smax = query_to_judge(1, n);
  	
  	if(smax == 1 || query_to_judge(1, smax) != smax) {
  		int L = smax, R = n + 1;
  		while(L + 1 < R) {
  			int m = L + ((R - L) >> 1);
  			if(query_to_judge(smax, m) == smax) R = m;
  			else L = m;
  		}
  		...
  		while(L + 1 < R) {
  			int m = L + ((R - L) >> 1);
  			if(query_to_judge(m, smax) == smax) L = m;
  			else R = m;
  		}
