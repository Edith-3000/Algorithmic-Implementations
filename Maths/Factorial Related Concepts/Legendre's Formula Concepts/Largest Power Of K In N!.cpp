/* NOTE: The largest power of k in n!, when k is a prime number has been discussed in --->
         "Legendre’s Formula Implementation.cpp"
   
   Prob: Given two numbers k and n, find the largest power of k that divides n!, where k may not 
         necessarily a prime number i.e. k > 1

   Ref: https://www.geeksforgeeks.org/largest-power-k-n-factorial-k-may-not-prime/
*/
...
// Time complexity: O(sqrt(k) + log(log(k)) * log(n)), because you use O(sqrt(k)) to get prime divisors
//                  of k, and the number of distinct prime divisors of k is approximately log(log(k)) also
//                  for each prime divisor the legendre_formula() takes O(log(n)) time. 
//                  Base of log can be considered roughly as 2. since it is the smallest prime.