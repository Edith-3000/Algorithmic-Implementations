...
int LLSU(string &s) {
	...
	while(j < n) {
		...
		if((int)cnt.size() == (j - i + 1)) {
			res = max(res, j - i + 1);
			j++;
		}
		
		else {
			while((int)cnt.size() < (j - i + 1)) {
				cnt[s[i]] -= 1;
...
// TC: O(n)
// NOTE: Although asymptotically time complexity is O(n). 
//       But in the worst case iterator 'i' will also traverse all string elements, so in order to avoid
//       the movement of 'i' by just one step until a valid window is not found we can smartly store the 
