...
void helper(vi op, vi &ip, int idx, int n, set<vi> &st) {
	if(idx >= n) {
	...
	helper(op1, ip, idx, n, st);
	helper(op2, ip, idx, n, st);
}

vvi unique_subsets(vi &v) {
	int n = sz(v);
	vi op;
	set<vi> st;
...
void helper(vi &op, vi &ip, int idx, int n, vvi &res) {
	res.pb(op);
	
	for(int i = idx; i < n; i++) {
		// if there is duplicate and it is already taken into account, skip it
		if((i != idx) and (ip[i] == ip[i - 1])) continue;
		
		op.pb(ip[i]); 
		helper(op, ip, i + 1, n, res);
		op.ppb();
	}
}
...
vvi unique_subsets(vi &v) {
	...
	// sorting is required here so that if there are duplicates, then they form 
	// a continuous subsegment
	sort(v.begin(), v.end());
