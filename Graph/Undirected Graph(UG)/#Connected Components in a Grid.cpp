...
void dfs(int cur_row, int cur_col, vvi &v) {
    ...
    for(int k = 0; k < 4; k++) {
        int nx = cur_row + dx[k], ny = cur_col + dy[k];
        if(is_valid(nx, ny) and !vis[nx][ny] and (v[nx][ny])) {
            dfs(nx, ny, v);
        }
    }
}
...
int count_connected_components(vvi &v) {
	vis.clear();
	vis.resize(n, vi(m));
...
void solve()
{
  	...
  	int res = count_connected_components(v);
}
