// Prob: https://www.geeksforgeeks.org/largest-bst-binary-tree-set-2/

/****************************************************************************************************************************************************************/

// METHOD - 1 (O(n^2))
// Ref: https://www.techiedelight.com/find-size-largest-bst-in-binary-tree/
...
void is_bst(TreeNode *root, TreeNode* &prev, bool &ok) {
	// base case
	if(root == NULL) return;
	
	is_bst(root->left, prev, ok);
...
bool check_bst(TreeNode *root) {
	// initially assuming that the tree is a BST
	bool ok = 1;
...
int cnt_nodes(TreeNode *root) {
	if(root == NULL) return 0;
	return 1 + cnt_nodes(root->left) + cnt_nodes(root->right);
}
...
int largest_bst(TreeNode *root) {
	...
	if(check_bst(root)) return cnt_nodes(root);
...
// METHOD - 2 (O(n))
// Ref: https://www.geeksforgeeks.org/largest-bst-binary-tree-set-2/
...
// METHOD - 3 [O(n)]
// Ref: https://www.youtube.com/watch?v=X0oXMdtUDwo&ab_channel=takeUforward
...
struct info {
	// size of subtree rooted at the current root
	int sz; 
	...
	// maximum value in the subtree rooted at the current root, if subtree rooted at current root is a BST, else it will contain INT_MAX
	int mx; 
...
info largest_bst(TreeNode *root) {
	...
	if((lf.mx < root->val) and (root->val < rg.mn)) {
		return {lf.sz + 1 + rg.sz, min(lf.mn, root->val), max(rg.mx, root->val)};
	}
	
	else return {max(lf.sz, rg.sz), INT_MIN, INT_MAX};
}
...
/*
* This is a method which is more aligned with the approach used in  "Largest Size BST in a BT.cpp" for these type of problems.
...
struct info {
	// size of subtree rooted at the current root, if subtree rooted at current root is a BST, else it will contain 0
	int sz; 
...
info largest_bst(TreeNode *root, int &res) {
	// base case
	if(root == NULL) return {0, INT_MAX, INT_MIN};
	
	info lf = largest_bst(root->left, res);
	info rg = largest_bst(root->right, res);

	res = max(res, lf.sz);
	res = max(res, rg.sz);
	
	if((lf.mx < root->val) and (root->val < rg.mn)) {
		res = max(res, lf.sz + 1 + rg.sz);
		return {lf.sz + 1 + rg.sz, min(lf.mn, root->val), max(rg.mx, root->val)};
	}
