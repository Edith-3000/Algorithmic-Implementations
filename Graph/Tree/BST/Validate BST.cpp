...
void solve()
{
	...
	if(check_bst(root, LLONG_MIN, LLONG_MAX)) cout << "Yes\n";
	else cout << "No\n";
}
...
// NOTE: prev is to be passed by reference
bool is_bst(TreeNode *root, TreeNode* &prev) {
	// base case
	if(root == NULL) return 1;
	
	// return false if left subtree is not a valid BST
	if(is_bst(root->left, prev) == 0) return 0;
	
	// if the current node satisfies BST property
	if((prev != NULL) and (root->val <= prev->val)) return 0;
	
	// update prev pointer before moving on to right subtree
	prev = root;
	
	// return false if right subtree is not a valid BST
	if(is_bst(root->right, prev) == 0) return 0;
