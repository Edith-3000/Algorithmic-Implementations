...
bool path_exists(TreeNode* root, int k, vi &res) {
	...
	if(path_exists(root->left, k, res) or 
...
vi get_path(TreeNode* root, int k) {
	vi res;
	if(root == NULL) return res;
	
	path_exists(root, k, res);
...
void solve()
{
	...
	vi res = get_path(root, k);
	
	if(sz(res) == 0) {
		cout << "Path does not exist or Root NULL\n";
	}
