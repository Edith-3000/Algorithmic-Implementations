// Prob: https://www.geeksforgeeks.org/print-unique-rows/
/*********************************************************************************************************/
...
bool trie_insert(Trie *root, vi &v) {
	Trie *cur = root;
	bool flag = 1;
	
	for(int i = 0; i < sz(v); i++) {
		if(cur->nxt[v[i]] == NULL) cur->nxt[v[i]] = new Trie(), flag = 0;
		cur = cur->nxt[v[i]];
	}
...
vvi unique_rows(vvi &v) {
	...
	for(int i = 0; i < n; i++) {
		if(trie_insert(root, v[i]) == 0) res.pb(v[i]);
	}
