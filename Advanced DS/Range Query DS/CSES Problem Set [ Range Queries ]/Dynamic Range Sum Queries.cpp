...
// Memory Limit: 512 MB
// Time Limit: 1000 ms
// Parsed on: 13-03-2021 11:00:06 IST (UTC+05:30)
...
ll query_seg_tree(int tl, int tr, int ql, int qr, int ind) {
	if(tl >= ql and tr <= qr) return t[ind];
	if(tr < ql || tl > qr) return 0LL;
	
	int mid = (tl + tr) / 2;
	ll left = query_seg_tree(tl, mid, ql, qr, 2 * ind);
	ll right = query_seg_tree(mid + 1, tr, ql, qr, 2 * ind + 1);
	
	return left + right;
}

void point_update_seg_tree(int tl, int tr, int p, int change, int ind) {
	if(p < tl || p > tr) return;
	if(tl == tr) {
		t[ind] = change;
		return;
	...
	int mid = (tl + tr) / 2;
	point_update_seg_tree(tl, mid, p, change, 2 * ind);
	point_update_seg_tree(mid + 1, tr, p, change, 2 * ind + 1);
...
void solve()
{
  	...
  	while(q--) {
  		...
  		if(type == 1) {
  			int k, u; cin >> k >> u;
  			k--;
  			point_update_seg_tree(0, n - 1, k, u, 1);
  		}
  		
  		else {
  			int a, b; cin >> a >> b;
  			a--, b--;
  			cout << query_seg_tree(0, n - 1, a, b, 1) << "\n";
  		}
  	}
}
...
void solve()
{
    ...
    SegmentTree<ll> mySegmentTree(v, 0LL, sum);
    ...
    while(q--) {
      ...
      if(type == 1) {
        int k, u; cin >> k >> u;
        k--;
        mySegmentTree.update(k, u);
      }
      
      else {
        int a, b; cin >> a >> b;
        a--, b--;
        cout << mySegmentTree.query(a, b) << "\n";
      }
    }
}
