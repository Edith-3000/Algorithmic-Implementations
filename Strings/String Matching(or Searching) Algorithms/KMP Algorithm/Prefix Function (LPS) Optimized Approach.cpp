...
/* # The KMP matching algorithm uses degenerating property (pattern having the same sub-patterns appearing more than once in the pattern) of the pattern 
     and improves the worst-case complexity to O(n). 

   # The basic idea behind KMP’s algorithm is: whenever we detect a mismatch (after some matches), we already know some of the characters in the text of the 
...
vi prefix_function_optimized(string &s) {
	...
	// i ==> normal iterator
	// j ==> basically denotes the length of the longest proper
	...
	while(i < n) {
		if(s[i] == s[j]) {
			...
			// find the longest prefix which was also the suffix in the suffix
			if(j == 0) {
				lps[i] = 0;
				i++;
			}
...
void solve()
{
  	...
  	vi lps = prefix_function_optimized(s);
...
// NOTE: BELOW CODE IS ALSO SAME AS ABOVE WITH FEW LINES OF CODE
