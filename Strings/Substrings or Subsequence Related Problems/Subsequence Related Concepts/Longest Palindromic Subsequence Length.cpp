...
int len_LPS(string &s, int i, int j) {
    // base case(s)
    if(i > j) return 0;
    if(i == j) return 1;
    ...
    if(s[i] == s[j]) res = 2 + len_LPS(s, i + 1, j - 1);
    else res = max(len_LPS(s, i, j - 1), len_LPS(s, i + 1, j));
...
// UNDERLYING CONCEPT :--->
// Longest Palindromic Subsequence i.e. LPS(s) = LCS(s, reverse(s))
