...
// LEGACY CONTENT: https://pastebin.com/qgYP6Z6M
...
int max_profit(int idx, int canBuy, int n, vi &v) {
	// base case(s)
	if(idx == n) return 0;
	...
	if(canBuy) {
		int bought = -v[idx] + max_profit(idx + 1, 0, n, v);
		int notBought = max_profit(idx + 1, 1, n, v);
		
		res = max(bought, notBought);
	}
	
	else {
		int sold = v[idx] + max_profit(idx + 1, 1, n, v);
		int notSold = max_profit(idx + 1, 0, n, v);
...
// Space complexity: O(n), [recursion call stack overhead]
...
// METHOD - 5 (USING VARIABLES INSTEAD OF std::vector IN METHOD - 4)
...
// METHOD - 6 (PEAK-VALLEY APPROACH)
// https://www.youtube.com/watch?v=K8iHi8AW1ls&ab_channel=TECHDOSE
