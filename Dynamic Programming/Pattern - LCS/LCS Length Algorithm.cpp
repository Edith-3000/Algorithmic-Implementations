...
int len_LCS(string &s1, string &s2, int n, int m) {
	...
	if(s1[n-1] == s2[m-1]) return 1 + len_LCS(s1, s2, n - 1, m - 1);
	
	// if last characters of both strings do not matches
	else return max(len_LCS(s1, s2, n, m - 1), 
	                len_LCS(s1, s2, n - 1, m));
}
...
int len_LCS(string &s1, string &s2, int n, int m) {
	...
	// choice diagram code
	// if last characters of both strings matches
	if(s1[n-1] == s2[m-1]) return dp[n][m] = 1 + len_LCS(s1, s2, n - 1, m - 1);
	
	// if last characters of both strings do not matches
	else return dp[n][m] = max(len_LCS(s1, s2, n, m - 1), 
	                           len_LCS(s1, s2, n - 1, m));
}
...
// dp[][] is a 2 D global matrix/vector of vectors, with size (n+1) x (m+1) and initialized 
