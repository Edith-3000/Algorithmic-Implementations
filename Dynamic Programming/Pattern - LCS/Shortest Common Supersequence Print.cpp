...
string print_SCS(string &s1, string &s2, int n, int m) {
    dp.clear();
    ...
    // starting from right bottom cell of dp[][]
    int i = n, j = m;
    ...
    while(i > 0 and j > 0) {
        ...
        // if current character in s1 and s2 are different 
            ...
            if(dp[i][j-1] >= dp[i-1][j]) { res.pb(s2[j-1]); j--; }
            
            // put current character of s1 in result 
            else { res.pb(s1[i-1]); i--; }
        }
    }
    
    // put remaining characters of s1 & s2 in the result string 
    while(j > 0) { res.pb(s2[j-1]); j--; }
