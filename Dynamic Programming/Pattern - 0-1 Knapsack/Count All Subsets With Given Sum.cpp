...
// https://youtu.be/zoilQD1kYSg?list=PLgUwDviBIf0qUlt5H_kiKYaNSqJ81PMMY&t=9
...
// this function handles the case of 0's' as well
int cnt_subset(int idx, int sum, vi &v) {
    // base case(s)
    if(idx == 0) {
        if((sum == 0) and (v[idx] == 0)) return 2;
        else if((sum == 0) or (sum == v[idx])) return 1;
        else return 0;
    }
    
    int exc = cnt_subset(idx - 1, sum, v);
    int inc = 0;
    
    if(v[idx] <= sum) inc = cnt_subset(idx - 1, sum - v[idx], v);
...
// METHOD - 3
// This is similar to "METHOD - 2", but here the case of '0' is handled in the loop only.
