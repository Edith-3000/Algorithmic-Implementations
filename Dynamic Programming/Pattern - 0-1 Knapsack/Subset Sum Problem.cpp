/* Link: https://www.geeksforgeeks.org/subset-sum-problem-dp-25/
   ...
   Ref: https://www.youtube.com/watch?v=_gPcYovP7wc&list=PL_z_8CaSLPWekqhdCPmFohncHwz8TY2Go&index=7
*/
...
bool is_subset(vi &v, int sum, int n) {
    ...
    // choice diagram code
    bool exc = is_subset(v, sum, n - 1);
    bool inc = false;
    
    if(v[n - 1] <= sum) inc = is_subset(v, sum - v[n - 1], n - 1);
...
/* Approach very much similar to "0 - 1 Knapsack Algorithm.cpp", 
   wt[] ---> v
   w ---> sum
