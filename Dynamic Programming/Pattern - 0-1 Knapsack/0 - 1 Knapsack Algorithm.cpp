...
ll zero_one_knapsack(vi &wt, vi &val, ll w, int n) {
	// base condition(s)
...
void solve()
{
 	int n; ll w; cin >> n >> w;
  	...
  	cout << zero_one_knapsack(wt, val, w, n) << "\n"; 
}
...
void solve()
{
 	int n; ll w; cin >> n >> w;
  	...
  	dp.resize(n+1);
  	for(int i = 0; i <= n; i++) dp[i].resize(w+1, -1);
...
// dp[][] is a 2 D global matrix/vector of vectors, with size (n+1) x (w+1) and initialized with -1, 
...
void solve()
{
 	int n; ll w; cin >> n >> w;
  	...
  	dp.resize(n+1);
  	for(int i = 0; i <= n; i++) dp[i].resize(w+1);
...
// dp[][] is a 2 D global matrix/vector of vectors, with size (n+1) x (w+1), where n is the size 
// of wt[] & val[] array.
...
ll zero_one_knapsack(vi &wt, vi &val, ll w, int n) {
    // space optimized dp[] vector
    vll prev(w + 1), cur(w + 1);
    
    for(int i = 0; i <= w; i++) prev[i] = 0LL;
    
    for(int i = 0; i < n; i++) {
        for(int j = 0; j <= w; j++) {
            ll exc = prev[j];
            ll inc = LLONG_MIN;
            if(wt[i] <= j) inc = val[i] + prev[j - wt[i]];
            cur[j] = max(exc, inc);
        }
...
ll zero_one_knapsack(vi &wt, vi &val, ll w, int n) {
    ...
    for(int i = 0; i <= w; i++) dp[i] = 0LL;
    
    for(int i = 0; i < n; i++) {
        for(int j = w; j >= 0; j--) {
            ll exc = dp[j];
            ll inc = LLONG_MIN;
            if(wt[i] <= j) inc = val[i] + dp[j - wt[i]];
            dp[j] = max(exc, inc);
        }
    }
...
// Time complexity: O(n x w)
// Space complexity: O(w), Although asymptotically the space is same as above method.